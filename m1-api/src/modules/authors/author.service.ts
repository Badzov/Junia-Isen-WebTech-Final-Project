...
export class AuthorService {
  ...
  public async updateAuthorAverageRating(authorId: string): Promise<void> {
    ...
    const booksWithReviews = authorBooks.filter((book) => book.averageRating > 0);

    // Calculate the average rating only for books with reviews
    const totalRatings = booksWithReviews.reduce((sum, book) => sum + book.averageRating, 0);
  ...
  public async updateNumberOfBooksWritten(authorId: string): Promise<void> {
    const books = await this.bookRepository.getBooks(); 
    const numberOfBooksWritten = books.filter((book) => book.authorId === authorId).length;
  
    await this.authorRepository.updateAuthor(authorId, { numberOfBooksWritten }); 
  }
}
